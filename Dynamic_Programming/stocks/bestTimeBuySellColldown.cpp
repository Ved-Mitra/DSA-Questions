...
int maxProfit_Memoization(vector<int> &prices,int i,bool buy,vector<vector<int>> &dp,int n)
{
    ...
    {
        int take=prices[i]+maxProfit_Memoization(prices,i+2,true,dp,n);//sell (including the cooldown period)
...
int maxProfit_Tabulation(vector<int> &prices)
{
    ...
    for(int i=n-1;i>=0;i--)
    {
        for(int j=1;j>=0;j--)
        {
            ...
            {
                int sell=prices[i]+dp[i+2][1];//including cooldown period
...
{
    int n=prices.size();
    vector<int> ahead(2,0),curr(2,0),ahead_ahead(2,0);
    ...
    for(int i=n-1;i>=0;i--)
    {
        for(int j=1;j>=0;j--)
        {
            ...
            {
                int sell=prices[i]+ahead_ahead[1];
                int notSell=ahead[0];
                profit=max(sell,notSell);
            }
            curr[j]=profit;
